...
//日志统计
var _alog=(function(window,undefined){function mbb_get_cookie(name){var mn=name+"=";var b,e;var co=document.cookie;if(mn=="="){return co;}
b=co.indexOf(mn);if(b<0){return"";}
e=co.indexOf(";",b+name.length);if(e<0){return co.substring(b+name.length+1);}else{return co.substring(b+name.length+1,e);}}
function mbb_element_click(activity,element){if(activity===undefined||activity===""||element===undefined||element==="")return;var data="activity="+encodeURIComponent(activity);data+="&element="+encodeURIComponent(element);data+="&session="+mbb_get_cookie("mbbsessionid");$.ajax({url:'http://xinmnt.mbaobao.com/click',data:data,type:"GET",dataType:"jsonp",async:true});}
return{log:mbb_element_click}})(this);
...
function add_goods_to_favorites(goods_sku,isAlert) {
    if(typeof isAlert == "undefined"){
        isAlert = true;
    }
    var url = APPS_URL + 'item/ajax/do/add_favourite.html?goods_sku=' + goods_sku + '&d=' + time();
    $.ajax({
        async: true,
        type: 'get',
        url: url,
